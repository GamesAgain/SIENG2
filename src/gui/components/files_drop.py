...
class FilesDropWidget(QFrame):
    ...
    def init_ui(self, text, sub_text, icon_path):
        ...
        self.icon_label = QLabel()
        self.icon_label.setPixmap(create_icon_pixmap(icon_path or str(ICON_DIR / "upload.svg"), size=30))
    ...
    def add_files(self, file_paths: list[str]):
        ...
        # Single File Mode: เลือกทับไฟล์เดิม (clear ก่อนเพิ่ม)
        if self.is_single_mode and file_paths:
            file_paths = [file_paths[0]]  # เอาแค่ไฟล์แรก
            if not self.is_allowed_file(file_paths[0].lower()):
                return
            # Clear ของเดิมออกก่อน (ถ้ามี)
    ...
    def render_single_preview(self, file_path):
        ...
        if file_ext in image_exts and self._preview_pixmap is not None:
            ...
            self.sub_label.hide()
            preview_padding = 10
            self.drop_layout.setContentsMargins(preview_padding, preview_padding, preview_padding, preview_padding)

            target_w = max(1, self.drop_zone.width() - (preview_padding * 2))
            target_h = max(1, self.drop_zone.height() - (preview_padding * 2))
